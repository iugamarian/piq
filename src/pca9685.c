...
void pca9685_setup(int freq)
{
    ...
    /* set frequency */
    pca9685_set_pwm_frequency(freq);
}
...
void pca9685_set_pwm_frequency(int freq)
{
    ...
    i2c_write_byte(PCA9685_PRE_SCALE, (int) prescale);
...
void pca9685_set_pwm(int8_t channel, int16_t off)
{
    ...
    i2c_set_slave(PCA9685_I2C_ADDR);
    i2c_write_byte(PCA9685_LED0_ON_L + (4 * channel), 0 & 0xFF);
    i2c_write_byte(PCA9685_LED0_ON_H + (4 * channel), 0 >> 8);
    i2c_write_byte(PCA9685_LED0_OFF_L + (4 * channel), off & 0xFF);
    i2c_write_byte(PCA9685_LED0_OFF_H + (4 * channel), off >> 8);
}
